...
describe("GET /auth/verify-email", () => {
    it("verifies email successfully with a valid token", async () => {
        ...
        const token = jwt.sign({ id: user.id, email: user.email }, process.env.APP_KEY, { expiresIn: "1h" });
    ...
    it("returns 404 if user from token does not exist", async () => {
        const fakeToken = jwt.sign({ id: "nonexistent-id", email: "ghost@example.com" }, process.env.APP_KEY, { expiresIn: "1h" });
        const res = await request(app)
...
//# sourceMappingURL=verify-email.test.js.map