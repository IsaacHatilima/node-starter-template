...
export class TwoFactorService {
    async initiateSetup(reqUser: Request) {
        const user = await prisma.user.findUnique({where: {id: reqUser.user.id}});
        if (!user)
    ...
    async verifyAndEnable(data: { code: string }, reqUser: Request) {
        const cached = await redis.get(`tfsetup:${reqUser.user.id}`);
        if (!cached)
    ...
    async disableMFA(data: { code: string, backup_code: string }, reqUser: Request) {
        const user = await prisma.user.findUnique({where: {id: reqUser.user.id}});
        if (!user)
            throw new Error("USER_NOT_FOUND");
